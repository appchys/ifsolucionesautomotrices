    ...
    <div className={`flex flex-col flex-1 min-h-full ${isSidebar ? "bg-slate-50" : ""}`}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 px-6 pb-4">
        ...
        <div className="w-full lg:flex-1 flex flex-col gap-4 min-w-0 lg:border-r lg:border-[var(--border)] pr-2">
            ...
            {/* Items Table */}
            <div className="border border-[var(--border)] rounded-xl bg-white overflow-hidden shadow-sm">
                      ...
                      {items.filter((it) => it.tipo === "servicio").length > 0 && (
                        <div>
                          ...
                          {items.filter((it) => it.tipo === "servicio").map((item) => {
                            ...
                            return (
                              <div key={item.id || idx} className="grid grid-cols-12 gap-1.5 sm:gap-2 p-2.5 sm:p-3 text-xs sm:text-sm border-b border-[var(--border)] items-center hover:bg-slate-50">
                                <div className="col-span-4 sm:col-span-5 font-semibold uppercase truncate text-xs sm:text-sm" title={item.descripcion}>{item.descripcion}</div>
                                <div className="col-span-3 sm:col-span-2 flex items-center justify-center gap-0.5 sm:gap-1">
                                  ...
                                  <input 
                                    type="number" 
                                    className="w-8 sm:w-10 text-center border border-[var(--border)] rounded py-0.5 px-0.5 text-xs font-mono" 
                                    value={item.cantidad} 
                                    onChange={(e) => {
                                ...
                                <div className="col-span-2 text-right">
                                  <input 
                                    type="number" 
                                    className="w-14 sm:w-16 text-right border border-[var(--border)] rounded py-0.5 px-1 text-xs font-mono" 
                                ...
                                <div className="col-span-1 text-center text-xs font-mono text-[var(--text-muted)]">{item.impuestoAplicable > 0 ? `${item.impuestoAplicable}%` : '0%'}</div>
